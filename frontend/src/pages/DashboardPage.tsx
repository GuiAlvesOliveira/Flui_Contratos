...
function KpiCard({ label, value, meta, icon }: KpiCardProps) {
  return (
...
function PhaseRow({ stage, count, total }: PhaseRowProps) {
...
export function DashboardPage({ onNavigate }: DashboardPageProps) {
  ...
  const activeProcesses = processes.filter(p =>
    MAIN_STAGES.includes(p.stage) && p.stage !== 'inicial'
  );
  const bankAnalysis = byStage('analise_credito');
  const emissaoJuridico = [...byStage('cartorio'), ...byStage('analise_juridica'), ...byStage('juridico_aprovado')];
  ...
  return (
    <div className="ds-page">
      <div className="ds-page-hdr">
        <div>
          <h1>Dashboard</h1>
          <p>Visão operacional consolidada · {processes.length} processos cadastrados</p>
        </div>
      ...
      <div className="ds-kpi-row">
        <KpiCard
          ...
          meta={`${processes.length} total cadastrados`}
          icon={<Icon.Activity size={12} />}
        />
        <KpiCard
          label="Em análise de crédito"
          value={isLoading ? '…' : bankAnalysis.length}
          meta="Recolhimento de documentos"
          icon={<Icon.Banknote size={12} />}
        />
        <KpiCard
          label="Jurídico / Cartório"
          value={isLoading ? '…' : emissaoJuridico.length}
          meta="Fase final do processo"
          icon={<Icon.CheckSquare size={12} />}
        />
        <KpiCard
          label="Docs para validar"
          value={docStats == null ? '…' : docStats.pending}
          meta={docStats ? `${docStats.total} docs no total` : 'Aguardando upload do cliente'}
      ...
      <div className="ds-dash-grid">
        ...
        <div className="ds-col-5">
          <div className="ds-card">
            <div className="ds-card-hdr">
              ...
              <span className="meta">{activeProcesses.length} ativos</span>
            </div>
                ...
                <div key={p.id} className="ds-alert warn">
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div className="ds-alert-desc">
                      {p.client.name ?? p.client.email} · {daysSince(p.updatedAt)}d sem atualização
                    </div>
                  </div>
                </div>
              ))}
              {byStage('credito_recusado').slice(0, 2).map((p) => (
                <div key={p.id} className="ds-alert urgent">
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div className="ds-alert-desc">
                      {p.client.name ?? p.client.email} · Verificar possibilidade de defesa
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="ds-col-8">
          <div className="ds-card">
            <div className="ds-card-hdr">
              <h3>Processos sem atualização recente</h3>
                ...
                {lateProcesses.map((p) => (
                  <div key={p.id} className="ds-proc-row">
                    <div>
                      ...
                      <div className="pr-sub">{p.analista ? `Analista: ${p.analista.name ?? p.analista.email}` : 'Sem analista'}</div>
                    </div>
          ...
          <div className="ds-card">
            ...
            <div className="ds-card-body" style={{ padding: 0 }}>
              <div className="ds-pri">
                ...
                <div style={{ flex: 1 }}>
                  <div className="ds-pri-title">Verificar pendências</div>
              ...
              <div className="ds-pri">
                ...
                <div style={{ flex: 1 }}>
                  <div className="ds-pri-title">Análise de crédito</div>
                  <div className="ds-pri-sub">{bankAnalysis.length} em coleta de documentos</div>
                </div>
              </div>
              <div className="ds-pri">
        ...
        {/* Timeline de atualizações recentes */}
                    ...
                    <div key={p.id} className="ds-tl-row">
                      ...
                      <div className="ds-tl-content">
                        ...
                        <div className="ds-tl-desc">
                          ...
                          {p.valorUnidade ? ` · ${formatCurrency(p.valorUnidade)}` : ''}
                          {p.analista ? ` · ${p.analista.name ?? p.analista.email}` : ''}
                        </div>
                      </div>
                    </div>
                  ))}
